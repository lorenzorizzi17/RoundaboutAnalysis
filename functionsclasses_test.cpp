...
TEST_CASE("Corretto funzionamento di .newcar_rbt()") {
  rbout rotonda(1, 1);
  road road_(M_PI / 2);
  rotonda.newcar_rbt((road_).angle(), 2);  // creo una car nella rotonda
                                           // all'angolo della strada road_
  CHECK((rotonda.size_rbout()) == 1);
  CHECK(
      ((rotonda.carrbout())[0]).theta() ==
      (M_PI / 2 +
       offset));  // controllo che la car venga istanziata nel punto corretto
}

TEST_CASE("Corretto avanzamento nella rotonda") {
  rbout roundabout(2, 1);
  roundabout.newcar_rbt(M_PI / 2, 2);  // istanzio una car a 90°
  for (int i = 0; i < 100; i++) {      // la faccio muovere nella rotonda
    (roundabout).evolve_rbt();
  }
  CHECK(((roundabout.carrbout())[0]).theta() ==
        doctest::Approx((M_PI / 2 + offset +
...
TEST_CASE(
  ...
  roundabout.newcar_rbt(
      M_PI / 2 - 0.1644,
      2);  // Creo una macchina in rotonda che si trovi ad un angolo pari a
...
TEST_CASE(
  ...
  SUBCASE(
      ...
      "rotonda") {
    roundabout.newcar_rbt(
        0., 2);  // creo volutamente una macchina in rotonda a theta=0
    ...
    CHECK((((road_).carin())[0]).t() ==
          doctest::Approx(dist_from_rbout).epsilon(0.01));  // ne verifico la posizione
  }

  SUBCASE(
      "La macchina entra in rotonda se non ci sono macchine in prossimità "
      "dell'uscita") {
    roundabout.newcar_rbt(2., 2);  // stavolta la macchina in rotonda è distante
                                   // e non c'è bisogno di darle la precedenza
    ((road_).carin()).push_back(C);
    for (int i = 0; i < 100;
    ...
    if (road_.transfer_rd()) {  // quando è il caso, trasferisco la macchina in
                                // strada in rotonda
      roundabout.newcar_rbt(road_.angle(), 2);
    }
    CHECK((road_).size_in() ==
